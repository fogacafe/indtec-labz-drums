...
export function App() {
  ...
  const [speedPercent, setSpeedPercent] = useState(100); const [metronomeEnabled, setMetronomeEnabled] = useState(false); const [audioStatus, setAudioStatus] = useState<AudioStatus>('idle');
  ...
  async function importXml(file: File) { try { loadXml(await file.text()); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not import MusicXML.'); } }
  ...
  return <main className="shell">
    ...
    <section className="control-deck"><div className="transport-main"><button className="restart-button" type="button" disabled={!chart} onClick={() => { setPlaying(false); setCurrentBeat(0); resetPerformance(); }}>↺</button><button className="play-button" type="button" disabled={!chart} onClick={() => void togglePlaying()}>{playing ? 'Ⅱ' : '▶'}<span>{playing ? 'Pause' : 'Play'}</span></button><button className={`metro-button ${metronomeEnabled ? 'active' : ''}`} type="button" onClick={() => void toggleMetronome()}><span>♩</span><div><small>METRONOME</small><strong>{metronomeEnabled ? 'ON' : 'OFF'}</strong></div></button></div><div className="speed-strip"><span>PRACTICE SPEED</span><div>{SPEED_OPTIONS.map((speed) => <button key={speed} type="button" className={speedPercent === speed ? 'active' : ''} onClick={() => setSpeedPercent(speed)}>{speed}%</button>)}</div></div><div className="audio-state">Audio {audioStatus}</div></section>
