...
export function App() {
  ...
  function updateLiveScore(content: string) { try { const next = parseMusicXml(content); setXml(content); setChart(next); setCurrentBeat(0); setError(null); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not render the live take.'); } }
  function previewRecordedTake(content: string) { loadXml(content); setMode('practice'); }
