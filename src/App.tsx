...
export function App() {
  ...
  function resetPerformance() {
    ...
    setHits({ perfect: 0, good: 0, earlyLate: 0, wrong: 0 });
    setFeedback(null);
    setKitSignal(null);
  }

  function loadXml(content: string) {
    const next = parseMusicXml(content);
    setXml(content);
    setChart(next);
    setCurrentBeat(0);
    setLoop(null);
  ...
  async function importScore(file: File) {
    try { loadXml(await readMusicXmlFile(file)); }
  ...
  function handleDrumHit(hit: DrumHit) {
    ...
    if (mode === 'studio' || !playing || !chart || startedAtRef.current === null) {
  ...
  function seekToBeat(beat: number) {
    if (!chart) return;
    const next = Math.min(Math.max(beat, 0), chart.totalBeats);
    setCurrentBeat(next);
    startedAtRef.current = performance.now() - next * beatDurationMs;
    matchedHitsRef.current.clear();
    setFeedback(null); setKitSignal(null);
  }

  async function togglePlaying() {
    if (!chart) return;
    if (!playing) {
      if (!loop && currentBeat >= chart.totalBeats - 0.001) { setCurrentBeat(0); startedAtRef.current = null; resetPerformance(); }
      if (metronomeEnabled) {
  ...
  async function toggleMetronome() {
    if (!metronomeEnabled) {
      try { const context = await ensureAudioContext(); scheduleMetronomeClick(context, context.currentTime, true); setMetronomeEnabled(true); }
      catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not start audio.'); }
      return;
    }
  ...
  function selectMeasure(measure: number) {
    setLoop((current) => !current || current.startMeasure !== current.endMeasure
      ? { startMeasure: measure, endMeasure: measure }
      : { startMeasure: Math.min(current.startMeasure, measure), endMeasure: Math.max(current.startMeasure, measure) });
  }

  useEffect(() => {
    if (!playing || !chart || mode !== 'practice') return;
    let frame = 0;
    ...
    const tick = (now: number) => {
      ...
      if (loop) {
        ...
        if (beat >= endBeat && loopLength > 0) { beat = startBeat + ((beat - startBeat) % loopLength); startedAtRef.current = now - beat * beatDurationMs; matchedHitsRef.current.clear(); }
      } else if (beat >= chart.totalBeats) {
        if (replayEnabled && chart.totalBeats > 0) { beat %= chart.totalBeats; startedAtRef.current = now - beat * beatDurationMs; matchedHitsRef.current.clear(); }
        else { beat = chart.totalBeats; setPlaying(false); }
      }
      setCurrentBeat(beat);
  ...
  }, [playing, chart, loop, replayEnabled, beatDurationMs, mode]);

  useEffect(() => {
    if (!playing || !metronomeEnabled || !chart || mode !== 'practice') return;
    const context = audioContextRef.current;
    if (!context || context.state !== 'running') return;
    const seconds = 60 / effectiveBpm;
    const currentWhole = Math.ceil(currentBeat - 0.0001);
    let nextBeat = currentWhole;
    let nextClickAt = context.currentTime + Math.max(Math.max(currentWhole - currentBeat, 0) * seconds, 0.03);
    const scheduler = window.setInterval(() => {
      if (context.state !== 'running') return;
      while (nextClickAt < context.currentTime + 0.12) { scheduleMetronomeClick(context, nextClickAt, nextBeat % chart.beatsPerMeasure === 0); nextBeat += 1; nextClickAt += seconds; }
    }, 25);
  ...
  return <main className={`shell mode-${mode}`}>
    <nav className="topbar">
      ...
      <div className="mode-switch" aria-label="App mode">
        <button className={isPractice ? 'active' : ''} onClick={() => changeMode('practice')}>Practice</button>
        <button className={!isPractice ? 'active' : ''} onClick={() => changeMode('studio')}>Studio</button>
      </div>
      <MidiMonitor onHit={handleDrumHit} />
    </nav>

    <header className="session-header">
      <div>
        <span className="eyebrow">{isPractice ? 'PRACTICE SESSION' : 'STUDIO SESSION'}</span>
        <h1>{isPractice ? chart?.title ?? 'Pick something to play' : chart?.title ?? 'Play it. We write it.'}</h1>
        <p>{isPractice
          ? chart ? `${chart.measures.length} measures · ${chart.beatsPerMeasure}/${chart.beatType} · original ${chart.bpm} BPM · ${chart.expectedHits.length} hits` : 'Load a score and turn notation into a playable practice lane.'
          : chart ? `${chart.measures.length} measures · ${chart.expectedHits.length} captured hits · live MusicXML` : 'Set the BPM, hit Record and create a score directly from your kit.'}</p>
      </div>
      {isPractice && <div className="hero-actions"><button className="demo-button" disabled={loadingDemo} onClick={() => void loadDemo()}>{loadingDemo ? 'Loading…' : 'Load demo'}</button><label className="import-button">Import score<input type="file" accept=".xml,.musicxml,.mxl" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importScore(file); }} /></label></div>}
    </header>

    {error && <div className="error">{error}</div>}

    <section className={`practice-stage ${playing ? 'is-playing' : ''} ${feedback && isPractice ? `feedback-${feedback.kind}` : ''}`} style={{ '--beat-duration': `${beatDurationMs}ms` } as React.CSSProperties}>
      <div className="stage-topline">
        <div className="tempo-readout"><span>TEMPO</span><strong>{chart ? Math.round(isPractice ? effectiveBpm : chart.bpm) : '—'}</strong><small>BPM</small></div>
        {isPractice ? <div className="beat-dots">{Array.from({ length: chart?.beatsPerMeasure ?? 4 }, (_, index) => <i key={index} className={chart && index === beatInMeasure ? 'active' : ''} />)}</div> : <div className="studio-live"><i /> LIVE SCORE</div>}
        <div className="loop-readout"><span>{isPractice ? 'LOOP' : 'TAKE'}</span><strong>{isPractice ? loop ? `${loop.startMeasure}—${loop.endMeasure}` : replayEnabled ? 'Replay chart' : 'Full chart' : chart ? `${chart.expectedHits.length} hits` : 'Ready'}</strong></div>
      </div>
      <div className="score-stage"><ScoreViewer xml={xml} currentBeat={isPractice ? currentBeat : 0} totalBeats={chart?.totalBeats ?? 0} beatsPerMeasure={chart?.beatsPerMeasure ?? 4} playing={isPractice && playing} onSeek={isPractice ? seekToBeat : undefined} />{feedback && isPractice && <div className={`hit-feedback ${feedback.kind}`}><strong>{feedback.label}</strong><span>{feedback.detail}</span></div>}</div>
      <DrumKitFeedback played={kitSignal?.played ?? null} expected={isPractice ? kitSignal?.expected ?? null : null} wrong={isPractice ? kitSignal?.wrong ?? false : false} />
      <div className="stage-status"><span className={isPractice && playing ? 'live-dot active' : !isPractice ? 'live-dot studio' : 'live-dot'} />{isPractice ? playing ? 'LISTENING TO YOUR KIT' : chart ? 'READY' : 'LOAD A SCORE' : 'MIDI INPUT READY'}</div>
    </section>

    {isPractice ? <>
      <section className="control-deck"><div className="transport-main"><button className="restart-button" disabled={!chart} onClick={() => { setPlaying(false); setCurrentBeat(0); resetPerformance(); }}>↺</button><button className="play-button" disabled={!chart} onClick={() => void togglePlaying()}>{playing ? 'Ⅱ' : '▶'}<span>{playing ? 'Pause' : 'Play'}</span></button><button className={`deck-button ${metronomeEnabled ? 'active' : ''}`} onClick={() => void toggleMetronome()}><span className="deck-icon">♩</span><span className="deck-copy"><small>METRONOME</small><strong>{metronomeEnabled ? 'ON' : 'OFF'}</strong></span></button><button className={`deck-button ${replayEnabled ? 'active' : ''}`} onClick={() => setReplayEnabled((value) => !value)}><span className="deck-icon">↻</span><span className="deck-copy"><small>REPLAY</small><strong>{replayEnabled ? 'ON' : 'OFF'}</strong></span></button></div><div className="speed-strip"><span>PRACTICE SPEED</span><div>{SPEED_OPTIONS.map((speed) => <button key={speed} className={speedPercent === speed ? 'active' : ''} onClick={() => setSpeedPercent(speed)}>{speed}%</button>)}</div></div><div className="audio-state">Audio {audioStatus}</div></section>
      ...
      <section className="timeline-section"><div className="timeline-title"><span>ARRANGEMENT</span><button disabled={!loop} onClick={() => setLoop(null)}>Clear loop</button></div><PracticeTimeline chart={chart} currentBeat={currentBeat} loop={loop} onSelectMeasure={selectMeasure} /></section>
    </> : <section className="studio-console"><Recorder bpm={chart?.bpm ?? 120} onRecordingChange={(handler) => { recordHandlerRef.current = handler; }} onPreview={loadXml} /></section>}
  </main>;
}
