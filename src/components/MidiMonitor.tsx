...
export function MidiMonitor({ onHit }: Props) {
  ...
  return <div className="midi-dock">
    <button className={`midi-pill ${connected ? 'connected' : ''}`} type="button" onClick={() => connected ? setExpanded((value) => !value) : void handleConnect()} disabled={!supported || connecting}>
      <span className="midi-dot" /><span>{connecting ? 'Connecting…' : connected ? devices[0]?.name ?? 'Drums connected' : 'Connect drums'}</span>
    </button>
